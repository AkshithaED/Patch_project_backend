...
from .views import (
    ...
    NestedPatchListView, NestedProductListView, NestedImageListView, SecurityIssueByImageView, SoftDeletePatchView, UpdatePatchView
)
...
urlpatterns = [
    path('', include(router.urls)),
    path('patches/delete/<str:pk>/', SoftDeletePatchView.as_view(), name='soft-delete-patch'), 
    path('patches/update/<str:pk>/', UpdatePatchView.as_view(), name='update-patch'),
    path('<str:release_name>/', NestedPatchListView.as_view()),
    path('<str:release_name>/<str:patch_name>/', NestedProductListView.as_view()),
